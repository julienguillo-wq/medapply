...
export default function InstallationLayout() {
  ...
  const installProgress = useInstallationProgress();
  ...
  return (
    <div className="flex min-h-screen">
      <InstallationSidebar
        ...
        stepStatuses={installProgress.stepStatuses}
      ...
      <div className="flex-1 md:ml-[260px] flex flex-col min-h-screen">
        ...
        <header className="md:hidden sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 bg-gradient-to-br from-install to-install-dark rounded-lg flex items-center justify-center shadow-[0_2px_8px_rgba(13,148,136,0.25)]">
                <span className="text-white font-bold text-xs">M</span>
              </div>
              <span className="font-bold text-base tracking-tight">Installation</span>
            </div>
          </div>
        ...
        <main className="flex-1 px-4 py-6 md:px-14 md:py-10 max-w-[1100px]">
          <Outlet context={installProgress} />
        </main>
      </div>
    </div>
  );
